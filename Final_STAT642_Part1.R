...
## using variables transformed from Analysis 2
#------------------------------------------
## Oversampling

set.seed(114) # initialize random seed

train3 <- upSample(x = train2[ ,vars2], # training set with 15 variables (transformed)
                   y = train2$Revenue, # target
                   yname = "Revenue") # name of y variable

## Undersampling was also attempted.
#train3 <- downSample(x = train2[ ,vars2], 
#                   y = train2$Revenue,
#                   yname = "Revenue") 

par(mfrow = c(1,2)) 

plot(train2$Revenue, main = "Original")

plot(train3$Revenue, main = "RUS")
...
nb3.train <- predict(object = nb_mod3, # NB model with downsample
                     newdata = train3[ ,vars2], # predictors
                     type = "class")
head(nb3.train)
...
## Testing Performance
#------------------------------------------
nb3.test <- predict(object = nb_mod3, # NB model
                    newdata = test2[ ,vars2], # predictors
                    type = "class")
...
#------------------------------------------
# Overall
cbind(Training3 = train_conf3$overall,
      Testing3 = test_conf3$overall) 

# Class-Level
cbind(Training3 = train_conf3$byClass,
      Testing3 = test_conf3$byClass)
